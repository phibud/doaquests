...
function event_say(e)
    ...
    eq.debug("Player Name: " .. player_name .. " Familiar: " .. tostring(familiar))
    ...
    if familiar then
        if e.message:findi("hail") then
            if qglobals[flag_key] == nil then
                ...
                if flag_value=="3" then
                    e.self:Say("Your inventory, spells, skills, AAs, discs should be updated.  If you would like to flag your account for another [sync] please let me know.  I can also set up your [sharedbank] sync for your account if you would like.")
                end
        ...
        elseif e.message:findi("sharedbank") then
            if qglobals["SharedBankSync"]==nil then
                e.self:Say("Your shared bank will be synced shortly.  If your shared bank slots are not empty, they will not be overwritten.")
                eq.set_global("SharedBankSync", "1", 5, "F")    
            elseif qglobals["SharedBankSync"]=="1" then
                e.self:Say("Your shared bank is already queued for sync.  Please wait for the process to complete.")
            elseif qglobals["SharedBankSync"]=="2" then
                e.self:Say("Your shared bank has already been synced.  If you are still having issues, please open a /bugreport")
            end
