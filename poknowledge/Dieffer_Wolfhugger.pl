...
sub EVENT_ITEM {
	...
	if (plugin::check_handin(\%itemcount, 85062 => 1) || plugin::check_handin(\%itemcount, 124688 => 1)) { # (85062) Bristlebane's Ticket of Admission  (124688) Peace Be With You
		$random_result = 0;
		...
		# (124688) Peace Be With You 
			...
			while($found_prize == 0){
				...
				if($loop_count > 100){
					...
					plugin::check_handin(\%itemcount, 124688 => 1);
					$found_prize = 1;
				}
